/**
 * Native file save for the HarmonyOS (ArkWeb) build.
 *
 * Problem: the stock "download from browser" flow (fetch -> blob: URL ->
 * `<a download>.click()`) is a no-op inside ArkWeb — the Web component has
 * no download handling for `blob:` URLs / the `download` attribute.
 *
 * Fix: the app's Index page injects a small purpose-built bridge
 * (`window.harmonyNative`, see entry/src/main/ets/pages/Index.ets). It saves
 * /api/download responses straight into the public Download directory
 * (Download/<bundleName>/ via the picker in DOWNLOAD mode), so the file is
 * visible in the system file manager — unlike the app sandbox (el2 data
 * dir), which third-party file managers cannot list.
 *
 * Layout of the chain, best first:
 *   1. window.harmonyNative - works on the Node-served UI page (Harmony app)
 *   2. blob anchor click     - desktop browser
 *
...
// ---------------------------------------------------------------------------
// window.harmonyNative bridge plumbing
// ---------------------------------------------------------------------------
...
function getHarmonyNative () {
  try {
    const native = window.harmonyNative
...
// Called from ArkTS (NativeBridge.report) on the UI thread.
...
function callHarmonyNative (method, args) {
  const native = getHarmonyNative()
  if (!native) return Promise.reject(new Error('harmonyNative unavailable'))
...
// The stock `<a download>` flow is a silent no-op inside ArkWeb.
// Detect it via the UA so we can show an error instead of
// pretending the download worked.
function isArkWeb () {
  try {
    return /HarmonyOS|ArkWeb/i.test(window.navigator.userAgent || '')
  } catch (e) {
    return false
  }
}
...
export async function canSaveNative () {
  return !!getHarmonyNative()
}

// Diagnostic helper for on-device debugging:
...
export async function downloadDiag () {
  ...
  const info = {
    hasHarmonyNative: !!native,
    harmonyNativeVersion: nativeVersion,
    isArkWeb: isArkWeb(),
    // Compat keys for consumers synced from electerm-android
    // (file-select-dialog.jsx checks these to detect a WebView where the
    // blob-anchor fallback is a silent no-op):
    isAndroidWebView: isArkWeb(),
    isNativePlatform: !!native,
    userAgent: (window.navigator && window.navigator.userAgent) || ''
  }
  try {
    console.log('[electerm-harmony] downloadDiag:', JSON.stringify(info))
  } catch (e) {}
  return info
}

export async function saveBlobNative (filename, blob, contentType) {
  if (!getHarmonyNative()) throw new Error('native file saver unavailable')
  const type = contentType || blob.type || guessContentType(filename)
  const buf = new Uint8Array(await blob.arrayBuffer())
  const res = await callHarmonyNative('saveBase64', [filename, uint8ToBase64(buf), type])
...
export async function saveTextNative (filename, text) {
  ...
  const res = await callHarmonyNative('saveBase64', [
    filename,
    uint8ToBase64(bytes),
...
/**
 * Replacement for the broken-in-ArkWeb blob-anchor download.
 * Fetches /api/download (token header included) and saves the result to
 * the public Download directory natively when running in the Harmony app.
 * On a plain desktop browser it falls back to the classic blob-anchor
 * download. Inside ArkWeb WITHOUT the native saver it reports an error
 * instead of silently no-op anchor clicking.
 */
export async function downloadPathFromServer (serverPath) {
  ...
  // 1. Purpose-built native bridge. Fetches the response natively, so the
  //    file never has to pass through JS memory.
  if (getHarmonyNative()) {
    try {
      const res = await callHarmonyNative('saveUrl', [
        absoluteUrl(url),
        currentToken(),
        fallbackName
      ])
      ...
      console.log('[electerm-harmony] native save failed:', err)
  ...
  // inside ArkWeb it is a silent no-op, so report instead of
  // pretending the download happened.
  if (isArkWeb()) {
    downloadDiag()
    message.error('Download failed: native file saver unavailable. Check Downloads / retry.')
    return
  }
...
export function installDownloadFromBrowserHook () {
  ...
  // Log the download chain state once at startup so `hdc hilog` /
